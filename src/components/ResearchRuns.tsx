...
const STATUS_STYLE: Record<string, string> = {
  RUNNING: 'text-verdict-maybe',
  SUCCESS: 'text-verdict-apply',
  FAILED: 'text-verdict-skip',
};
      ...
      {runs.length === 0 ? (
        ...
        <ul className="space-y-3">
          {runs.map(run => {
            ...
            return (
              <li key={run.id} className="rounded-control border border-line px-3 py-2.5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="text-caption text-ink-muted">
                    {run.startedAt.toLocaleString('en-US')} · {run.triggeredBy}
                  </span>
                  <span className={`text-caption font-semibold ${STATUS_STYLE[run.status]}`}>
                    {run.status.toLowerCase()}
                    {run.status === 'SUCCESS' && !run.grounded && ' (unverified)'}
                  </span>
                </div>

                {run.error && <p className="mt-1 text-caption text-verdict-skip">{run.error}</p>}

                {sources.length > 0 && (
                  <ul className="mt-2 space-y-0.5">
                    {sources.slice(0, 6).map(source => (
                      <li key={source.url} className="truncate text-caption">
                        <a
                          href={source.url}
                          target="_blank"
                ...
                {criteria && (
                  <details className="mt-2">
                    ...
                    <pre className="mt-2 max-h-64 overflow-auto rounded bg-band p-2 text-caption leading-relaxed">
